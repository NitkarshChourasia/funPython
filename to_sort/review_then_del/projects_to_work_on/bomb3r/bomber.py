...
parser.add_argument(
    "--sms", "-S", type=int, help="Number of sms to target (default: 20)", default=20
)
parser.add_argument(
    "--country",
    "-c",
...
parser.add_argument(
    ...
    "-T",
    type=int,
    help="Max number of concurrent HTTP(s) requests (default: 20)",
    default=20,
)
parser.add_argument(
    "--proxy",
...
if args.verify:
    ...
    with ThreadPoolExecutor(max_workers=no_of_threads) as executor:
        for i in range(no_of_sms):
            p = Provider(
                target, proxy=proxies, verbose=args.verbose, cc=str(args.country)
            )
