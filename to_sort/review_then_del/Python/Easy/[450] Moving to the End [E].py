...
For example, if the given arrays is {1, 9, 8, 4, 0, 0, 2, 7, 0, 6, 0}, it …
_________
_________
...
# Requirements:
# - To append all the selected element.


# Your code should go here:

def move_to_end(lst: list, to_move) -> list:
    elements_to_move = [ele for ele in lst if ele == to_move]
    remaining_elements = [ele for ele in lst if ele != to_move]
    output = remaining_elements + elements_to_move
    return output


print(move_to_end([1, 3, 2, 4, 4, 1], 1))  # ➞ [3, 2, 4, 4, 1, 1]

print(move_to_end([9, 7, 8, 9, 1, 2, 3, 4], 9))  # ➞ [7, 8, 1, 2, 3, 4, 9, 9]
