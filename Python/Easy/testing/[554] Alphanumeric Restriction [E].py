"""
##Alphanumeric Restriction

Create a function that returns True if the given string has any of the following:
___
*) Only letters and no numbers.
*) Only numbers and no letters.
___

If a string has both numbers and letters, or contains characters which don't fit into any category, return False


[Examples]

___
alphanumeric_restriction("Bold") ➞ True

alphanumeric_restriction("123454321") ➞ True

alphanumeric_restriction("H3LL0") ➞ False

alphanumeric_restriction("ed@bit") ➞ False
_____



[Notes]

Any string that contains spaces or is empty should return False.


[language_fundamentals] [regex] [strings] [validation] 



-------------------------------------------------------------------
[Resources]
_________
isalpha() Method
https://www.geeksforgeeks.org/python-string-isalpha-application/
Returns “True” if all characters in the string are alphabets, Otherwise, It returns “False”.
_________
_________
isdigit() Method
https://www.geeksforgeeks.org/python-string-isdigit-application/
Returns “True” if all characters in the string are digits.
_________
_________
isnumeric() Method
https://www.programiz.com/python-programming/methods/string/isnumeric
Returns True if all characters in a string are numeric characters. If not, it returns False.
_________
_________
Python 3 Basic Operators
https://www.tutorialspoint.com/python3/python_basic_operators.htm
Operators are the constructs, which can manipulate the value of operands.
_________
""" 
# Your code should go here:

def alphanumericRestriction(str1):
    if str1.isaplha():
        return True
    elif str1.isnumeric():
        return True
    else:
        return False

print(alphanumericRestriction("Bold")) # ➞ True

print(alphanumericRestriction("123454321")) # ➞ True

print(alphanumericRestriction("H3LL0")) # ➞ False

print(alphanumericRestriction("ed@bit")) # ➞ False


# testing.
# checkResources.