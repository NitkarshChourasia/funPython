...
A factor chain is a list where each previous element is a
...
def factorChain(lst1):
    i = 0
    trueLst1 = []
    while(i < lst1.len()):
        if lst1[i + 1] % lst1[i] == 0:  # Do the maths how many iteration it will take. But where and how to test it? Does debug lets you do that?
            trueLst1.append(True)
        elif lst1[i + 1] % lst1[i] == 1:
            trueLst1.append(False)
        i = i + 1
    if trueLst1.all():
        return True
    else:
...
print(factorChain([3, 6, 12, 36]))

# 3 is a factor of 6
# 6 is a factor of 12
# 12 is a factor of 36

print(factorChain([1, 2, 4, 8, 16, 32])) # ➞ True

print(factorChain([1, 1, 1, 1, 1, 1])) # ➞ True

print(factorChain([2, 4, 6, 7, 12])) # ➞ False

print(factorChain([10, 1])) # ➞ False


# testing.
# checkResources.
# check how to debug on pycharm, please.