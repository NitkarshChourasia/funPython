"""
##The DECIMATOR

Write a DECIMATOR function which takes a string and decimates it
(i.e. it removes the last 1/10 of the characters).
Always round up: if the string has 21 characters, 1/10 of the characters would be 2.1 characters,
hence the DECIMATOR removes 3 characters. The DECIMATOR shows no mercy!


[Examples]

___
DECIMATOR("1234567890") ➞ "123456789"
# 10 characters, removed 1.

DECIMATOR("1234567890AB") ➞ "1234567890"
# 12 characters, removed 2.

DECIMATOR("123") ➞ "12"
# 3 characters, removed 1.

DECIMATOR("123456789012345678901") ➞ "123456789012345678"
# 21 characters, removed 3.
_____



[Notes]

Make sure to remove characters from the end of the string.


[language_fundamentals] [strings] 



-------------------------------------------------------------------
[Resources]
_________
Cutting and Slicing Strings in Python
https://www.pythoncentral.io/cutting-and-slicing-strings-in-python/
An overview on all of the ways you can cut and slice strings with the Python programming language. With lots of examples/code samples!
_________
_________
How to Round Numbers in Python
https://www.knowledgehut.com/blog/programming/python-rounding-numbers
While you are dealing with data, sometimes you may come across a biased dataset. In statistics, bias is whereby the expected value of the results differs from the true …
_________
_________
How to Index and Slice Strings in Python
https://www.digitalocean.com/community/tutorials/how-to-index-and-slice-strings-in-python-3
The Python string data type is a sequence made up of one or more individual characters consisting of letters, numbers, whitespace characters, or symbols. Strings are se …
_________
_________
len() Method
https://www.guru99.com/python-string-length-len.html
You can use the len() to get the length of the given string, array, list, tuple, dictionary, etc.
_________
""" 
# Your code should go here:

def decimator(str1 : str) -> str:
    strSize = len(str1)
    toRemove = round(strSize * 0.1) ### Should be a round up.
    outputStr1 = str1[: -toRemove]
    return outputStr1

print(decimator("1234567890")) # ➞ "123456789"
# 10 characters, removed 1.

print(decimator("1234567890AB")) # ➞ "1234567890"
# 12 characters, removed 2.

print(decimator("123")) # ➞ "12"
# 3 characters, removed 1.

print(decimator("123456789012345678901")) # ➞ "123456789012345678"


# testing.
# checkResources.