...
Given a list of prices prices and a "supposed" total t, return True if there is a
hidden fee added to the total (i.e. the total is greater than the sum of prices), otherwise return False.
...
Removes any leading and trailing character arguments.
_________
_________
Sum() Function
...
Is used for iterating over a sequence (that is either a list, a tuple, a dictionary, a set, or a string). With the for loop we can execute a set of statements, once fo …
_________
_________
Spicing Strings
...
Hello everyone, am back to discuss a new python program. Here we learn how to sum all the elements in a list quite easily. We use a predefined function called sum() and …
_________
"""
# Your code should go here:


def hasHiddenFee(prices, t):
    if prices.sum == t:
        return False
    else:
        return True
...
print(hasHiddenFee((["$2", "$4", "$1", "$8"], "$15")) # ➞ False
print(hasHiddenFee((["$1", "$2", "$3"], "$6")) #  ➞ False
print(hasHiddenFee((["$1"], "$4")) # ➞ True
