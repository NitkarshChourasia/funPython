"""
##Reverse Coding Challenge #3

This is a reverse coding challenge. Normally you're given explicit directions with how to create a function.
Here, you must generate your own function to satisfy the relationship between the inputs and outputs.
Your task is to create a function that, when fed the inputs below, produces the sample outputs shown.


[Examples]

___
[5, 7, 8, 2, 1], 2 ➞ [1, 1, 0, 0, 1]

[9, 8, 16, 47], 4 ➞ [1, 0, 0, 3]

[17, 11, 99, 55, 23, 1], 5 ➞ [2, 1, 4, 0, 3, 1]

[6, 1], 7 ➞ [6, 1]

[3, 2, 9], 3 ➞ [0, 2, 0]

[48, 22, 0, 19, 33, 100], 10 ➞ [8, 2, 0, 9, 3, 0]
_____



[Notes]

If you get stuck, see Comments for a hint.


[arrays] [loops] [math]



-------------------------------------------------------------------
[Resources]
_________
% modulus
https://python-reference.readthedocs.io/en/latest/docs/operators/modulus.html
Returns the decimal part (remainder) of the quotient.
_________
"""
# Your code should go here:


def binOddEven(lst1, n):
    outputAns = [] # Is there any alternative to this.
    for i in lst1:
        outputAns.append(i%n)
    return outputAns

print([5, 7, 8, 2, 1], 2)) # ➞ [1, 1, 0, 0, 1]
print([9, 8, 16, 47], 4)) # ➞ [1, 0, 0, 3]
print([17, 11, 99, 55, 23, 1], 5)) # ➞ [2, 1, 4, 0, 3, 1]
print([6, 1], 7)) # ➞ [6, 1]
print([3, 2, 9], 3 )) # ➞ [0, 2, 0]
print([48, 22, 0, 19, 33, 100], 10 )) # ➞ [8, 2, 0, 9, 3, 0]

# testing.
