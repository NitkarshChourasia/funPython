"""
##Unlucky 13

Given a sorted list of numbers, remove any numbers that are divisible by 13. Return the amended list.


[Examples]

___
unlucky_13([53, 182, 435, 591, 637]) ➞ [53, 435, 591]
# 182 and 637 are divisible by 13.

unlucky_13([24, 316, 393, 458, 1279]) ➞ [24, 316, 393, 458, 1279]
# No numbers in the list are divisible by 13.

unlucky_13([104, 351, 455, 806, 871]) ➞ []
# All numbers in the list are divisible by 13.
_____



[Notes]

N/A


[arrays] [language_fundamentals] [numbers]



-------------------------------------------------------------------
[Resources]
_________
Remove Multiple Elements from a List
https://www.geeksforgeeks.org/remove-multiple-elements-from-a-list-in-python/
Given a list of numbers, write a Python program to remove multiple elements from a list based on the given condition.
_________
_________
Fastest way to check if a number is divisible by another?
https://stackoverflow.com/questions/52773914/fastest-way-to-check-if-a-number-is-divisible-by-another-in-python
You can simply use % Modulus operator to check divisibility. For example: n % 2 == 0 means n is exactly divisible by 2 and n % 2 != 0 means n is not exactly divisible by 2.
_________
_________
Data Structures
https://docs.python.org/3/tutorial/datastructures.html
This chapter describes some things you’ve learned about already in more detail, and adds some new things as well.
_________
_________
Lists Explained
http://openbookproject.net/thinkcs/python/english3e/lists.html
A list is an ordered collection of values. The values that make up a list are called its elements, or its items. We will use the term element or item to mean the same t …
_________
"""
# Your code should go here:


def unlucky13(lst1):
    lst1 = []
    for i in lst1:
        if i % 13 != 0:
            lst1.append(i)
    return lst1


def unlucky13_2(lst1):
    for i in lst1:
        if i % 13 == 0:
            lst1.remove(i)
    return lst1


print(unlucky13([53, 182, 435, 591, 637])) # ➞ [53, 435, 591]
# 182 and 637 are divisible by 13.
print(unlucky13([24, 316, 393, 458, 1279])) # ➞ [24, 316, 393, 458, 1279]
# No numbers in the list are divisible by 13.
print(unlucky13([104, 351, 455, 806, 871])) # ➞ []
# All numbers in the list are divisible by 13.

print("By second method: ")

print(unlucky13_2([53, 182, 435, 591, 637])) # ➞ [53, 435, 591]
# 182 and 637 are divisible by 13.
print(unlucky13_2([24, 316, 393, 458, 1279])) # ➞ [24, 316, 393, 458, 1279]
# No numbers in the list are divisible by 13.
print(unlucky13_2([104, 351, 455, 806, 871])) # ➞ []
# All numbers in the list are divisible by 13.


# testing.
# checkAgain.
