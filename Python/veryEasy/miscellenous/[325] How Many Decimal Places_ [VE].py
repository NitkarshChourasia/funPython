"""
##How Many Decimal Places?

Create a function that returns the number of decimal places a number has. Any zeros after the decimal point count towards the number of decimal places.


[Examples]

___
get_decimal_places("43.20") ➞ 2

get_decimal_places("400") ➞ 0

get_decimal_places("3.1") ➞ 1
_____



[Notes]

Return 0 if the number doesn't have any decimal places (see example #2).


[numbers] [regex] [strings]



-------------------------------------------------------------------
[Resources]
_________
find() Method
https://www.programiz.com/python-programming/methods/string/find
Returns the index of first occurrence of the substring (if found). If not found, it returns -1.
_________
_________
len() Method
https://www.programiz.com/python-programming/methods/built-in/len
Returns the number of items (length) in an object.
_________
_________
split() Method
https://www.geeksforgeeks.org/python-string-split/
Returns a list of strings after breaking the given string by the specified separator.
_________
"""
# Your code should go here:


def getDecimalPlaces(input1):
    count = input1.split(".")
    number = count.len()
    return number


print(getDecimalPlaces("43.20")) # ➞ 2
print(getDecimalPlaces("400")) # ➞ 0
print(getDecimalPlaces("3.1")) # ➞ 1


# testing.
# incomplete.
# checkAgain. The resources are must.

