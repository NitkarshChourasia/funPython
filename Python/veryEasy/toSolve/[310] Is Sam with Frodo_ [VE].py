"""
##Is Sam with Frodo?

Sam and Frodo need to be close. If they are side by side in the list, your function should return True. If there is a name between them, return False.


[Examples]

___
middle_earth(["Frodo", "Sam", "Gandalf"]) ➞ True

middle_earth(["Frodo", "Saruman", "Sam"]) ➞ False

middle_earth(["Orc", "Sam", "Frodo", "Legolas"]) ➞ True
_____



[Notes]

___
*) No matter who comes first, the result must be True if Frodo and Sam are side by side.
*) There is only one Sam and one Frodo in the list.
___



[algorithms] [arrays] [logic] [validation]



-------------------------------------------------------------------
[Resources]
_________
index() Method
https://www.tutorialspoint.com/python/list_index.htm
Returns the lowest index in list that obj appears.
_________
_________
Python Absolute Value
https://www.pythonpool.com/python-absolute-value/
In Python or any other Programming language, Python absolute value means to remove any negative sign in front of a number and to think of all numeric values as positive …
_________
"""
# Your code should go here:


def middleEarth(lst1):
    lst1 = lst1.lower()
    samIndex = lst1.index("sam")
    frodoIndex = lst1.index("frodo")
    if samIndex == 1+frodoIndex or frodoIndex == 1+samIndex:
        return True
    else:
        return False


print(middleEarth(["Frodo", "Sam", "Gandalf"])) # ➞ True
print(middleEarth(["Frodo", "Saruman", "Sam"])) # ➞ False
print(middleEarth(["Orc", "Sam", "Frodo", "Legolas"])) # ➞ True


# testing.
