...
def sumFoundIndexes(lst1, n):
    lst2 = []
    count = 0
    for i in lst1:
        if i == n:
            lst2.append(index(i)+ count)
            lst1.remove(i)
            count = count + 1
    return lst2.sum()


print(sumFoundIndexes([0, 3, 3, 0, 0, 3], 3)) # ➞ 8
# The number 3 was found at indexes 1, 2 and 5.
# 8 = 1 + 2 + 5
print(sumFoundIndexes([1, 2, 3, 4, 5, 6], 3)) # ➞ 2
print(sumFoundIndexes([100, 100, 100, 100, 100], 100)) # ➞ 10
print(sumFoundIndexes([5, 10, 15, 20], 2)) # ➞ 0


# testing.
# only check the resources if deem necessary, don't confuse yourself unnecessarily.
