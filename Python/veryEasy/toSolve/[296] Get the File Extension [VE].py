"""
##Get the File Extension

Write a function that maps files to their extension names.


[Examples]

___
get_extension(["code.html", "code.css"])
➞ ["html", "css"]

get_extension(["project1.jpg", "project1.pdf", "project1.mp3"])
➞ ["jpg", "pdf", "mp3"]

get_extension(["ruby.rb", "cplusplus.cpp", "python.py", "javascript.js"])
➞ ["rb", "cpp", "py", "js"]
_____



[Notes]

N/A


[arrays] [higher_order_functions] [strings]



-------------------------------------------------------------------
[Resources]
_________
split() Method
https://www.programiz.com/python-programming/methods/string/split
Breaks up a string at the specified separator and returns a list of strings.
_________
_________
rfind() Method
https://www.tutorialspoint.com/python3/string_rfind.htm
Returns the last index where the substring str is found, or -1 if no such index exists, optionally restricting the search to string[beg:end].
_________
_________
List Comprehensions
https://towardsdatascience.com/python-basics-list-comprehensions-631278f22c40
List comprehensions provide us with a simple way to create a list based on some iterable. During the creation, elements from the iterable can be conditionally included …
_________
"""
# Your code should go here:


def getExtension(lst1):
    outputAns = []
    for i in lst1:
        outputAns(i.split("."))
    return outputAns


print(getExtension((["code.html", "code.css"])))
# ➞ ["html", "css"]
print(getExtension((["project1.jpg", "project1.pdf", "project1.mp3"])))
# ➞ ["jpg", "pdf", "mp3"]
print(getExtension((["ruby.rb", "cplusplus.cpp", "python.py", "javascript.js"])))
# ➞ ["rb", "cpp", "py", "js"]


# testing.
